...
@dataclass(frozen=True, kw_only=True)
class TriggerSpec:
    """Generic trigger spec - fields depend on hook type.

    Each hook defines what fields its triggers can match against.
    Field names map to pattern sets that are matched using the
    appropriate matcher from the registry.
    """
...
Action = Annotated[
    SuggestSkillAction | RemindAction | BlockAction | InjectContextAction,
